...
function get_helium10_json(asin, marketplaceId) {
    var html_json = JSON.parse(get_content(`https://members.helium10.com/extension/calculator-json?price=10&asin=${asin}&marketplaceId=${marketplaceId}`, type = 'html'));
    if (html_json.status == false) {
        console.error('-> get_helium10_json -> 请先登录 helium10');
    } else {
        var image = html_json.product.AttributeSets.ItemAttributes.SmallImage.URL;
        image = image.replace('_SL75_.', '').replace('http://ecx.images-amazon.com', 'https://images-na.ssl-images-amazon.com');

        var title = html_json.product.AttributeSets.ItemAttributes.Title;
        var brand_name = html_json.product.AttributeSets.ItemAttributes.Brand;
        if (html_json.product.AttributeSets.ItemAttributes.hasOwnProperty('PackageDimensions')) {
            var package_height = Number(html_json.product.AttributeSets.ItemAttributes.PackageDimensions.Height).toFixed(1);
            var package_length = Number(html_json.product.AttributeSets.ItemAttributes.PackageDimensions.Length).toFixed(1);
            var package_width = Number(html_json.product.AttributeSets.ItemAttributes.PackageDimensions.Width).toFixed(1);
            var package_weight = Number(html_json.product.AttributeSets.ItemAttributes.PackageDimensions.Weight).toFixed(1);
            var package_dim = package_height + '*' + package_length + '*' + package_width;
        }
        if (html_json.product.AttributeSets.ItemAttributes.hasOwnProperty('ItemDimensions')) {
            var item_height = Number(html_json.product.AttributeSets.ItemAttributes.ItemDimensions.Height).toFixed(1);
            var item_length = Number(html_json.product.AttributeSets.ItemAttributes.ItemDimensions.Length).toFixed(1);
            var item_width = Number(html_json.product.AttributeSets.ItemAttributes.ItemDimensions.Width).toFixed(1);
            var item_weight = Number(html_json.product.AttributeSets.ItemAttributes.ItemDimensions.Weight).toFixed(1);
            var item_dim = item_height + '*' + item_length + '*' + item_width;
        }
        if (html_json.product.SalesRankings.SalesRank.length > 0) {
            var big_cat = html_json.product.SalesRankings.SalesRank[0].ProductCategoryId.split('_')[0];
            var big_bsr = html_json.product.SalesRankings.SalesRank[0].Rank;
        }

        var small_cat = '';
        var small_bsr = '';
        for (var i in html_json.bsrList) {
            small_cat += i + '\n';
            small_bsr += html_json.bsrList[i] + '\n';
        }

        for (var dict in html_json.feeEstimateFbaListArray) {
            var list = html_json.feeEstimateFbaListArray[dict];
            if (list.type == 'FBA Fulfillment Fees') {
                var fba_fee = list.amount;
            }
        }
        var information = [];
        information.push({ "key": "ASIN:", "value": typeof asin == 'undefined' ? 'null' : asin });
        information.push({ "key": "品牌名:", "value": typeof brand_name == 'undefined' ? 'null' : brand_name });
        information.push({ "key": "大类:", "value": typeof big_cat == 'undefined' ? 'null' : big_cat });
        information.push({ "key": "大类BSR:", "value": typeof big_bsr == 'undefined' ? 'null' : big_bsr });
        information.push({ "key": "小类:", "value": typeof small_cat == 'undefined' ? 'null' : small_cat });
        information.push({ "key": "小类BSR:", "value": typeof small_bsr == 'undefined' ? 'null' : small_bsr });
        information.push({ "key": "包装尺寸(inch):", "value": typeof package_dim == 'undefined' ? typeof item_dim == 'undefined' ? 'null' : item_dim : package_dim });
        information.push({ "key": "重量(lbs):", "value": typeof package_weight == 'undefined' ? typeof item_weight == 'undefined' ? 'null' : item_weight : package_weight });
        information.push({ "key": "FBA费用:", "value": typeof fba_fee == 'undefined' ? 'null' : fba_fee });

        var html_code = get_content(`https://members.helium10.com/black-box/sales-chart?days=0&asin=${asin}&marketplace=${marketplaceId}`, type = 'html');
        var json_code = /var data = (.*?);/.exec(html_code);
        if (json_code != null) {
            var html_json = JSON.parse(json_code[1]);
            if (html_json.status != "success") {
                console.error('-> get_helium10_json -> 请先登录 helium10');
            } else {
                var monthly_sales = {};
                var day_sales = {};
                ...
                var w3_sale = 0;
                var w4_sale = 0;
                var h24_sale = Number(html_json.sales[html_json.sales.length - 1].y);
                for (let sales_index in html_json.sales) {
                    var monthly = moment(new Date(html_json.sales[sales_index].x)).format('YYYY-MM');
                    var day_1 = moment(new Date(html_json.sales[sales_index].x)).format('YYYY-MM-DD');
                    var day_2 = moment(new Date()).format('YYYY-MM');
                    var sales = html_json.sales[sales_index].y;
                    monthly_sales[monthly] == undefined ? monthly_sales[monthly] = sales : monthly_sales[monthly] += sales;
                    day_sales[day_1] = sales;

                    var d1 = new Date().setTime(Number(html_json.sales[sales_index].x));
                    var d2 = new Date(new Date().getFullYear() + '-' + Number(new Date().getMonth() + 1) + '-' + new Date().getDate()).getTime();
                    var day_s = Number((d2 - d1) / 1000 / 60 / 60 / 24).toFixed(2);
                    switch (true) {
                        case day_s < 6.5:
                            w1_sale += Number(html_json.sales[sales_index].y);
                            continue;
                        case day_s < 13.5:
                            w2_sale += Number(html_json.sales[sales_index].y);
                            continue;
                        case day_s < 20.5:
                            w3_sale += Number(html_json.sales[sales_index].y);
                            continue;
                        case day_s < 27.5:
                            w4_sale += Number(html_json.sales[sales_index].y);
                            continue;
                    }
                }

                var h24_sale = w1_sale == 0 ? 0 : h24_sale;
                var sales_days = (new Date().setTime(Number(html_json.sales[html_json.sales.length - 1].x) - new Date().setTime(Number(html_json.sales[0].x))) / 1000 / 60 / 60 / 24);
                var monthly_sales_length = Object.keys(monthly_sales).length;
                var monthly_sales_value = Object.keys(monthly_sales).map(key => monthly_sales[key]);
                var day_sales_length = Object.keys(day_sales).length;
                var day_sales_value = Object.keys(day_sales).map(key => day_sales[key]);
                if (monthly_sales_length > 30) {
                    var monthly_sales_max = Math.max.apply(null, monthly_sales_value.slice(monthly_sales_length - 30));
                } else {
                    var monthly_sales_max = Math.max.apply(null, monthly_sales_value);
                }
                if (day_sales_length > 30) {
                    var day_sales_max = Math.max.apply(null, day_sales_value.slice(day_sales_length - 30));
                } else {
                    var day_sales_max = Math.max.apply(null, day_sales_value);
                }
            }
        }
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    ...
    marketplaceId = get_amazon_config(location.href).marketplaceID; // 获取 Amazon 站点的 marketplaceId
    keepa_market_id = get_amazon_config(location.href).keepa_market_id; // 获取 Amazon 站点的 keepa_market_id
    ...
    if (get_amazon_page_type('amazon')) {  // 判断是不是在 亚马逊 页面
        $("#a-page").prepend(get_template_html('template/amazon_page_menu.html', mode = 'body')); // 在亚马逊页面插入导航栏
    ...
    if (get_amazon_page_type('listing')) {  // 判断是不是在 亚马逊 Listing 页面
        var asin = $('#ASIN').attr('value');
        get_helium10_json(asin, marketplaceId);
        $('#feature-bullets').before(get_template_html('template/prodDetails.html'))
    }
    ...
    if (location.href.indexOf(`www.amazon`) > -0x1 && location.href.indexOf(`mod=asin_compare&asins=`) > -0x1) {
        if (new Date().getTime() >= new Date(exd)) {
            ...
            for (i = 0x0; i < asins_a.length; i++) {
                html_code = _0x395d81(`https://` + document.domain + `/gp/recs-more-like-this/ajax/more-like-this.html?asin=` + asins_a[i], '');
                image = html_code.split(`src="`)[0x1].split('"')[0x0].replace(`_SR108,108_AC_.`, '');
                title = html_code.split(`medium mlt-title">`)[0x1].split('<')[0x0];
                if (html_code.indexOf(`a-color-price">`) > -0x1) {
                    price = html_code.split(`a-color-price">`)[0x1].split('<')[0x0];
                } else {
                    price = '';
                }
                ...
                if (html_code.indexOf(`product-reviews/` + asins_a[i]) > -0x1) {
                    asin_length = html_code.split(`product-reviews/` + asins_a[i]).length - 0x1;
                    reviews = html_code.split(`product-reviews/` + asins_a[i])[asin_length].split('>')[0x1].split('<')[0x0];
                    rating = html_code.split(`a-icon-alt">`)[0x1].split(' ')[0x0].replace(',', '.');
                } else {
                    reviews = '';
                    rating = '';
                }
                similar_code = '[' + asins_a[i] + ';' + price + ';' + reviews + ';' + rating + ']';
                if (html_code.indexOf(`<div data-asin="`) > -0x1) {
                    similar_a = html_code.split(`<div data-asin="`);
        ...
        if (location.href.indexOf(`mod=asin_compare&info=sale`) > -0x1 && location.href.indexOf(`www.amazon`) > -0x1) {
            if (new Date().getTime() >= new Date(exd)) {
                ...
                for (i = 0x1; i < asins_a.length; i++) {
                    ...
                    html_code = _0x1d12a5(`https://members.helium10.com/extension/calculator-json?price=10&asin=` + asin + `&marketplaceId=` + marketplaceID, '');
                    if (html_code.indexOf(`{"status":false,"errorCode":2}`) > -0x1) {
                        alert(`请先登录helium10`);
                        open(`https://members.helium10.com`);
                        void 0x0;
                        break;
                    } else {
                        if (html_code.indexOf(`URL":"`) > -0x1) {
                            image = html_code.split(`URL":"`)[0x1].split('"')[0x0].replace(`_SL75_.`, '').replace(`http://ecx.images-amazon.com`, `https://images-na.ssl-images-amazon.com`);
                        } else {
                            image = '';
                        }
                        if (html_code.indexOf(`Title":"`) > -0x1) {
                            title = html_code.split(`Title":"`)[0x1].replace(/\"/g, '引号').split('"')[0x0].replace(/引号/g, '"');
                        } else {
                            title = '';
                        }
                        if (html_code.indexOf(`Brand":"`) > -0x1) {
                            brand_name = html_code.split(`Brand":"`)[0x1].split('"')[0x0];
                        } else {
                            brand_name = '';
                        }
                        if (html_code.indexOf(`_display_on_website`) > -0x1) {
                            big_cat = html_code.split(`ProductCategoryId":"`)[0x1].split(`_display_on_website`)[0x0];
                            big_bsr = html_code.split(`ProductCategoryId":"`)[0x1].split(`Rank":"`)[0x1].split('"')[0x0];
                        } else {
                            big_cat = '';
                            big_bsr = '';
                        }
                        if (html_code.indexOf(`bsrList":{"`) > -0x1) {
                            small_cat = html_code.split(`bsrList":{"`)[0x1].split('"')[0x0];
                            small_bsr = html_code.split(`bsrList":{"`)[0x1].split(':"')[0x1].split('"')[0x0];
                        } else {
                            small_cat = '';
                            small_bsr = '';
                        }
                        if (html_code.indexOf(`PackageDimensions":{`) > -0x1) {
                            package_temp = html_code.replace(/{"@attributes":{"Units":"inches"},"0":/g, '').replace(/{"@attributes":{"Units":"pounds"},"0":/g, '').split(`PackageDimensions":{`)[0x1].split('}')[0x0];
                        ...
                        if (html_code.indexOf(`ItemDimensions":{`) > -0x1) {
                            item_temp = html_code.replace(/{"@attributes":{"Units":"inches"},"0":/g, '').replace(/{"@attributes":{"Units":"pounds"},"0":/g, '').split(`ItemDimensions":{`)[0x1].split('}')[0x0];
                        ...
                        if (html_code.indexOf(`FBA Fulfillment Fees","amount":"`) > -0x1) {
                            fba_fee = html_code.split(`FBA Fulfillment Fees","amount":"`)[0x1].split('"')[0x0];
                        } else {
                            fba_fee = '';
                        }
                        h24_sale = 0x0;
                        ...
                        w3_sale = 0x0;
                        w4_sale = 0x0;
                        html_code = _0x1d12a5(`https://members.helium10.com/black-box/sales-chart?days=0&asin=` + asin + `&marketplace=` + marketplaceID, '');
                        sales_days = '';
                        if (html_code.split(`sales":[`)[0x1].split(']')[0x0] != '') {
                            _0x51eec0 = html_code.split(`sales":[`)[0x1].split(']')[0x0].split(`x":`);
                    ...
                    if (new Date().getTime() >= new Date(exd)) {
                        ...
                        for (i = 0x0; i < asin_a.length; i++) {
                            ...
                            if (html_code.indexOf(`{"status":false,"errorCode":2}`) > -0x1) {
                                ...
                                if (html_code.indexOf(`URL":"`) > -0x1) {
                                    image_url = html_code.split(`URL":"`)[0x1].split('"')[0x0].replace(`_SL75_.`, `_SL200_.`).replace(`http://ecx.images-amazon.com`, `https://images-na.ssl-images-amazon.com`);
                                } else {
                                    image_url = '';
                                }
                                if (html_code.indexOf(`Title":"`) > -0x1) {
    ...
    if (location.href.indexOf('q=') > -0x1 && location.href.indexOf(`www.sellersprite.com/favicon.ico`) > -0x1) {
        if (new Date().getTime() >= new Date(exd)) {
            ...
            for (k = 0x0; k < keywords_a.length; k++) {
                if (keywords_a[k] != '') {
                    html_code = _0x395d81(`https://www.sellersprite.com/v2/keyword/history-trend-month/` + site + `?q=` + keywords_a[k], '');
                    keyword_code = `<tr style="font-size:11px;">关键词: ` + decodeURI(keywords_a[k]) + `</tr>`;
                    searches_a = html_code.split(`"search","data":[`)[0x1].split(']')[0x0].split(',');
                    sales_a = html_code.split(`"purchase","data":[`)[0x1].split(']')[0x0].split(',');
                    rate_a = html_code.split(`"rate","data":[`)[0x1].split(']')[0x0].split(',');
                    time_a = html_code.split(`"time","data":[`)[0x1].split(']')[0x0].split(',');
        ...
        if (location.href.indexOf(`keyword=`) > -0x1 && location.href.indexOf(`completion.amazon`) > -0x1) {
            if (new Date().getTime() >= new Date(exd)) {
                ...
                for (k = 0x0; k < keywords_a.length; k++) {
                    if (keywords_a[k] != '') {
                        html_code = _0x395d81(`https://` + document.domain + `/api/2017/suggestions?site-variant=desktop&mid=` + marketplaceID + `&alias=aps&limit=13&prefix=` + keywords_a[k]);
                        if (display_code.indexOf(decodeURI(keywords_a[k])) < 0x0) {
                            display_code = display_code + (k == 0x0 ? '' : `<br>`) + decodeURI(keywords_a[k]);
                        }
                        if (html_code.indexOf(`KEYWORD","value":"`) > -0x1) {
                            related_keywords_a = html_code.split(`KEYWORD","value":"`);
